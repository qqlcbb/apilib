...
class Tree{
    ...
    */
     ...
     * @return array
     */
    public function auto_tree($list, $pk='id', $pid = 'pid', $child = 'child')
    {
        $allIds = array_column($list, $pk);
        ...
        foreach($roots as &$root) {
            $root[$child] = $this->_getAutoTreeChild($root[$pk], $pk, $pid, $child);
        }
     ...
     * @return array
     */
    private function _getAutoTreeChild($id, $pk, $pid, $child)
    {
        $childArr = $this->_getChild($id, $pk, $pid, $child);

        if ($childArr) {
            foreach ($childArr as &$value) {
                $value[$child] = $this->_getAutoTreeChild($value[$pk], $pk, $pid, $child);
            }
        }
     ...
     * @return array
     */
    private function _getChild($id, $pk, $pid) {
        $childArr = [];

        foreach ($this->tempList as $item) {
